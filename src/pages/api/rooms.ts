...
=======
import toJson from "../../../../../../Desktop/bigIntToJson";
>>>>>>> Stashed changes
    ...
    switch (method) {
        ...
        case 'POST':
            ...
            if(body.short_break && ( body.short_break<3 || body.short_break>10 )){ 
                res.status(400).end("짧은 쉬는 시간은 3~10분 범위로만 설정할 수 있습니다.")
            }
            if(body.long_break && ( body.long_break<10 || body.long_break>30 )){ 
                res.status(400).end("긴 쉬는 시간은 10~30분으로만 설정할 수 있습니다.")
            }
            if(body.long_break_interval && ( body.long_break_interval<2 || body.long_break_interval>6 )){ 
                res.status(400).end("쉬는 시간 인터벌은 2~6회로만 설정할 수 있습니 다.")
            }
