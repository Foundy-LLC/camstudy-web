...
const FriendRecommendGroup: NextPage<{
  items: UserOverview[];
  setModal: (userId: string) => void;
}> = observer(({ items, setModal }) => {
  return (
...
const FriendRecommend: NextPage<{
  item: UserOverview;
  setModal: (userId: string) => void;
}> = observer(({ item, setModal }) => {
  const { friendStore } = useStores();
    ...
    <>
      ...
      >
        ...
        >
          <div style={{ display: "flex", flexDirection: "row" }}>
            <label
              className={`${friendStyles["friend-recommend__name"]} typography__text`}
            >
              {name}
            </label>
          </div>
          <label
            className={`${friendStyles["friend-recommend__introduce"]} typography__text`}
          >
            {introduce}
          </label>
        </div>
...
const friends: NextPage = observer(() => {
  ...
  useEffect(() => {
    if (userStore.currentUser) {
      ...
      friendStore.fetchRecommendFriend(userStore.currentUser.id);
    }
          ...
          <div style={{ display: "flex" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column" }}>
              <FriendRequestGroup
                ...
                setModal={setModal}
              />
              <FriendRecommendGroup
                items={friendStore.recommendedFriendOverviews}
                setModal={setModal}
              />
            </div>
          </div>
